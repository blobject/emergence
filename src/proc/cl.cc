...
Cl::PrepSeek()
{
  // see also: proc.cc/PlainSeek(),PlainSeekVicinity,PlainSeekTally()
  std::string code =
         ...
         int cols, int rows, unsigned int w, unsigned int h, float scope2)
{
  ...
  try
  {
    cl::Buffer G(this->context_, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,
                 cols * rows * gstride * sizeof(int), grid.data());
    ...
    this->kernel_seek_.setArg(15, static_cast<cl_int>(gstride));
